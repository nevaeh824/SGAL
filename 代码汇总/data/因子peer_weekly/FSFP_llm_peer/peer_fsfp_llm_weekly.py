...
def load_and_prepare_fsfp_data(fsfp_file, value_column="fsfp_original"):
    ...
    rename_map = {}
    if 'Stkcd' not in fsfp_df.columns and 'stock_code' in fsfp_df.columns:
        rename_map['stock_code'] = 'Stkcd'
    ...
    if 'Week' not in fsfp_df.columns and 'week_end' in fsfp_df.columns:
        week_end = pd.to_datetime(fsfp_df['week_end'], errors='raise')
        fsfp_df['Week'] = week_end.dt.strftime('%Y%W')

    if value_column not in fsfp_df.columns and value_column == "fsfp_original" and "fsfp" in fsfp_df.columns:
        value_column = "fsfp"
...
def main():
    ...
    """
    # 文件路径配置
    fsfp_file = "fsfp_weekly_corrected.csv"  # 包含Stkcd/code/stock_code, FSFP/fsfp_original/fsfp, Week/week/week_end列
    group_file = "data/business_communities_all_years.csv"  # 分组数据
    similarity_folder = "similarity_matrices"  # 相似度数据文件夹
    output_file = "peer_fsfp_llm_weekly_0509.csv"
